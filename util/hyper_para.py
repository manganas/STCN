...
class HyperParameters:
    def parse(self, unknown_arg_ok=False):
        ...
        # Data parameters
        parser.add_argument(
            "--static_root", help="Static training data root", default="../static"
        )
        parser.add_argument(
            "--bl_root", help="Blender training data root", default="../BL30K"
        )
        parser.add_argument(
            "--yv_root", help="YouTubeVOS data root", default="../YouTube"
        )
        parser.add_argument("--davis_root", help="DAVIS data root", default="../DAVIS")

        parser.add_argument(
            "--stage",
            help="Training stage (0-static images, 1-Blender dataset, 2-DAVIS+YouTubeVOS (300K), 3-DAVIS+YouTubeVOS (150K)), 4-DAVIS",
            type=int,
            default=0,
        )
        parser.add_argument(
            "--num_workers",
            help="Number of datalaoder workers per process",
            type=int,
            default=8,
        )
        ...
        parser.add_argument(
            "-b",
            "--batch_size",
            help="Default is dependent on the training stage, see below",
            default=None,
            type=int,
        )
        parser.add_argument(
        ...
        # Loading
        parser.add_argument(
            "--load_network", help="Path to pretrained network weight only"
        )
        parser.add_argument(
            "--load_model",
            help="Path to the model file, including network, optimizer and such",
        )

        # Logging information
        parser.add_argument(
            "--id",
            help="Experiment UNIQUE id, use NULL to disable logging to tensorboard",
            default="NULL",
        )
        parser.add_argument(
            "--debug",
            help="Debug mode which logs information more often",
            action="store_true",
        )

        # Multiprocessing parameters, not set by users
        parser.add_argument(
            "--local-rank", default=0, type=int, help="Local rank of this process"
        )

        ## Save paths
        parser.add_argument(
            "--save_model_path", help="Path to saved network", default="./saves/"
        )

        parser.add_argument(
            "--exp_name", help="Name of the dataset used", default="davis"
        )
