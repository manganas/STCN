...
class STCNModel:
    def __init__(self, para, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.optimizer = optim.Adam(
            filter(lambda p: p.requires_grad, self.STCN.parameters()),
            lr=para["lr"],
            weight_decay=1e-7,
        )
    ...
    def do_pass(self, data, it=0, current_epoch=0):
        ...
        # Plot the first of the batch to see

        # fig, ax = plt.subplots(2,3)
        # print(Fs[0].shape, Ms[0].shape)
        # return -1
        ...
        with torch.cuda.amp.autocast(enabled=self.para["amp"]):
            ...
            if self.single_object:
                ...
                prev_logits, prev_mask = self.STCN(
                    "segment",
                    k16[:, :, 1],
                    kf16_thin[:, 1],
                    kf8[:, 1],
                ...
                this_logits, this_mask = self.STCN(
                    "segment",
                    k16[:, :, 2],
                    kf16_thin[:, 2],
                    kf8[:, 2],
                ...
                ref_v1 = self.STCN(
                    "encode_value", Fs[:, 0], kf16[:, 0], Ms[:, 0], sec_Ms[:, 0]
                )
                ref_v2 = self.STCN(
                    "encode_value", Fs[:, 0], kf16[:, 0], sec_Ms[:, 0], Ms[:, 0]
                )
                    ...
                    if self._is_train:
                        if it % self.save_im_interval == 0 and it != 0:
                            if self.logger is not None:
                                ...
                                self.logger.log_cv2(
                                    "train/pairs",
                                    pool_pairs(images, size, self.single_object),
                                    it,
                                )
                ...
                if (it) % self.report_interval == 0 and it != 0:
                    if self.logger is not None:
                        ...
                        self.logger.log_metrics(
                            "train",
                            "time",
                            (time.time() - self.last_time) / self.report_interval,
                            it,
                        )
                    self.last_time = time.time()
